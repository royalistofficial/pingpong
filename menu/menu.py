...
class Menu:
    def __init__(self, font_size=74, font_color=(255, 255, 255), highlight_color=(255, 0, 0)):
    ...
    def draw(self, window):
        ...
        for index, option in enumerate(options):
            ...
            window.blit(text, (WIDTH // 2 - text.get_width() // 2, 10 + index * 100))

    def handle_input(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_UP:
                self.selected_option = (self.selected_option - 1) % len(self.get_current_options())
    ...
    def get_current_options(self):
        return self.main_menu_options if self.current_menu == "main" else self.difficulty_menu_options
